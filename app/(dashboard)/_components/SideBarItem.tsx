'use client';

import { usePathname, useRouter } from 'next/navigation';
import { cn } from '@/lib/utils';
import { LucideIcon } from 'lucide-react';

interface ISideBarItemProps {
  icon: LucideIcon;
  label: string;
  href: string;
}

function SideBarItem({ icon: Icon, label, href }: ISideBarItemProps) {
  const pathname = usePathname();
  const router = useRouter();

  const isActive =
    (pathname === '/' && href === '/') ||
    pathname === href ||
    pathname?.startsWith(`${href}/`);

  const onClick = () => {
    router.push(href);
  };

  return (
    <button
      className={cn(
        'flex items-center gap-x-2 text-slate-500 text-sm font-medium pl-6 transition-all hover:text-slate-600 hover:bg-slate-3-/20 cursor-pointer',
        isActive &&
          'text-orange-700 bg-orange-200/20 hover:bg-orange-200/20 hover:text-orange-700',
      )}
      onClick={onClick}>
      <div className=' flex items-center gap-x-2 py-4'>
        <Icon
          size={22}
          className={cn('text-slate-500', isActive && 'text-orange-700')}
        />
        {label}
      </div>

      <div
        className={cn(
          'ml-auto opacity-0 border-2 border-orange-700 h-full transition-all',
          isActive && 'opacity-100',
        )}
      />
    </button>
  );
}

export default SideBarItem;
